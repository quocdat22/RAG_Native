"""ChromaDB synchronization utility for syncing with Supabase."""
import logging
...
async def sync_chromadb_from_supabase(document_id: Optional[str] = None) -> dict:
    """
    Sync ChromaDB with Supabase documents.
    ...
    try:
        ...
        logger.info(f"🔄 Starting sync for {len(documents)} documents from Supabase to ChromaDB")
        ...
        for doc in documents:
            try:
                # Check if already in ChromaDB by checking chunk count
                ...
                try:
                    ...
                    # Add to ChromaDB with the original document ID
...
def sync_chromadb_from_supabase_sync(document_id: Optional[str] = None) -> dict:
    """Synchronous version of sync_chromadb_from_supabase for use in lifespan."""
    import asyncio
    try:
    ...
    return loop.run_until_complete(sync_chromadb_from_supabase(document_id))
